...
如果需要自定义构建，比如指定 `dist` 目录等，则需要通过 [config](https://github.com/mgbq/nx-admin/blob/master/config/index.js) 进行配置。

<br>

## 环境变量

所有测试环境或者正式环境变量的配置都在 [@/build/config](https://github.com/mgbq/nx-admin/tree/master/config) 目录之下
...
nx-admin 中，前端路由使用的是 `vue-router`，所以你可以选择两种方式：`browserHistory` 和 `hashHistory`。
...
相关[issue](https://github.com/mgbq/nx-admin/issues/370)
