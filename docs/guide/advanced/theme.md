...
Some global element-ui style modifications can be set in [@/src/styles/element-ui.scss](https://github.com/mgbq/nx-admin/blob/master/src/styles/element-ui.scss).
...
Code: [@/src/components/ThemePicker](https://github.com/mgbq/nx-admin/blob/master/src/components/ThemePicker/index.vue)。

**Briefly explain its principle:** All styles after element-ui version 2.0 are based on SCSS, all colors are set based on a few basic color [variables](https://github.com/mgbq/custom-element-theme/blob/master/element-variables.scss), so it is not difficult to achieve dynamic skinning, as long as find a few color variables to modify it. First, we need to get the version number of element-ui through `package.json` and request the corresponding style according to the version number. After you get the style, you will change the color, replace it with the color variable you want, and then dynamically add the `style` tag to override the original CSS style.
...
First download [custom-element-theme](https://github.com/mgbq/custom-element-theme)

```bash
git@github.com:mgbq/custom-element-theme.git
```
